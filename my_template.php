...
function my_userinfo($prefix = '') {
	...
	foreach($items as $it) {
		...
		// Using userlink() shows the full username instead and gives an indication of the currently logged in user account.
		if ($typ === 'profile') {
		    $screenreader_info = "<span class=\"sronly\">" . $lang['loggedinas'] . ' </span>';
		    $label = userlink();
		} else {
		    $screenreader_info = '';
		    $label = $it->getLabel();
		...
		echo $prefix . "<li class=\"action $typ\">" . $screenreader_info . "<a href=\"" . htmlentities($it->getLink()) . '" title="' . $it->getTitle() . '">' . $label . "</a></li>\n";
	}
